...
int main_test_diameter(const char *input_graph, int diameter_right) {
	...
	if (diameter_computed != diameter_right) {
		fprintf(stderr, "Diameters don't match, "
				"computed=%d, right=%d\n", diameter_computed, diameter_right);
...
int main(int argc, char *argv[]) {
	...
	if (argc == 5 && !strcmp(argv[1], "--optimize"))
		return (main_optimize(strtol(argv[2], &tmp_c, 10), argv[3], argv[4]));

	/* ./main --test-diameter graph.txt N */
	if (argc == 3 && !strcmp(argv[1], "--test-diameter"))
		return (main_test_diameter(argv[2], strtol(argv[3], &tmp_c, 10)));

	/* ./main --mpi graph.txt i_domination */
	if (argc == 4 && !strcmp(argv[1], "--mpi"))
		return (main_mpi(&argc, &argv, argv[2], strtol(argv[3], &tmp_c, 10)));

	/* ./main --seq graph.txt i_domination */
	if (argc == 4 && !strcmp(argv[1], "--seq"))
		return (main_seq(argv[2], strtol(argv[3], &tmp_c, 10)));
